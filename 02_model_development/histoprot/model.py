...
class HistoProt(nn.Module):
    """Histology-based Proteomic Profiler."""
    ...
    def forward_region(
        ...
        attention_logits = self.region_attention(patch_embeddings)  # (B, N, H)
        attention_logits = attention_logits.transpose(1, 2)         # (B, H, N)
    ...
    def forward_slide(
        ...
        # patch_embeddings: (B, N, D), region_attention: (B, H, N)
        attention_logits = self.slide_attention(patch_embeddings)  # (B, N, H)
        attention_logits = attention_logits.transpose(1, 2)        # (B, H, N)
    ...
    def forward(
        ...
        if not isinstance(patch_region_assignments, torch.Tensor):
            patch_region_assignments = torch.as_tensor(
                patch_region_assignments, dtype=torch.long, device=patch_features.device,
            )
        ...
        pooled_patch_predictions: list[torch.Tensor] = []

        for region_id in unique_region_ids:
            region_indices = torch.where(patch_region_assignments == region_id)[0]

            region_proteins, region_functions, region_attn = self.forward_region(
            ...
            region_attention_maps.append(region_attn)

            pooled = patch_level_predictions[:, region_indices, :].mean(dim=1)
            pooled_patch_predictions.append(pooled)

        pooled_patch_predictions_t = torch.cat(pooled_patch_predictions)
        ...
        slide_protein_predictions, slide_function_predictions = self.forward_slide(
            patch_embeddings, region_attention_tensor,
        )

        patch_to_region_loss = F.mse_loss(pooled_patch_predictions_t, region_level_predictions_t)
        mean_region_protein = region_level_predictions_t.mean(dim=0, keepdim=True)
        mean_region_function = region_level_function_predictions_t.mean(dim=0, keepdim=True)
        region_to_slide_protein_loss = F.mse_loss(mean_region_protein, slide_protein_predictions)
        region_to_slide_function_loss = F.mse_loss(mean_region_function, slide_function_predictions)
        hierarchical_consistency_loss = (
            patch_to_region_loss + region_to_slide_protein_loss + region_to_slide_function_loss
        )

        return {
            "slide_protein_predictions": slide_protein_predictions,
            "slide_function_predictions": slide_function_predictions,
            "hierarchical_consistency_loss": hierarchical_consistency_loss,
        }
